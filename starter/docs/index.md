...
function showReloadOverlay() {
  if (document.getElementById('tokken-reload-overlay')) return
...
async function waitForServer() {
  ...
  syncLog.value += '\n[error] Server did not come back. Refresh manually.'
  status.value = 'error'
}
...
async function submit() {
  ...
  try {
    ...
    if (syncLog.value.includes('Extraction complete') || syncLog.value.includes('Generating')) {
      status.value = 'done'
      await waitForServer()
    } else {
      status.value = 'error'
